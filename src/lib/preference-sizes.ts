...
/**
 * Разделителем считается только явный знак — двоеточие или тире, — но не
 * пробел.
 *
 * С пробелом в списке разделителей любая заметка, начинающаяся со слова,
 * совпадающего с именем категории, теряла это слово: «Обувь нужна тёплая»
 * превращалась в `Обувь: нужна тёплая`, а «Кольцо на мизинец» — в
 * `Кольцо: на мизинец`. Склейка всегда пишет двоеточие, поэтому канонические
 * строки распознаются как прежде; строка вида «Одежда M», набранная когда-то
 * в едином поле, теперь целиком уходит в «Другое» — на виду, а не порезанной.
 */
function categoryPrefixMatcher(name: string) {
  return new RegExp(`^${escapeRegExp(name)}\\s*[:\\-–—]\\s*`, "i");
}
