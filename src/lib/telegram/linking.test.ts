...
describe("telegram linking", () => {
  it("returns not_configured without telegramId", () => {
    expect(inferTelegramLinkStatus({ telegramId: null, telegramConfirmedAt: null })).toBe(
      "not_configured",
    );
  });

  it("returns pending when id exists but not confirmed", () => {
    expect(inferTelegramLinkStatus({ telegramId: "123456789", telegramConfirmedAt: null })).toBe(
      "pending",
    );
  });
