    ...
    >
      {chips.map((chip) => (
        <button
          ...
          className="group inline-flex min-w-0 max-w-full items-center gap-1.5 rounded-full border border-border/55 bg-[hsl(var(--surface-3)/0.55)] py-1 pl-2.5 pr-1.5 text-xs text-muted-foreground transition-colors hover:border-border hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
      ...
      <button
        type="button"
        onClick={onClearAll}
        className="ml-0.5 rounded-full px-2 py-1 text-xs font-medium text-muted-foreground underline-offset-4 transition-colors hover:text-foreground hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
      >
        {t("Сбросить всё")}
      </button>
    </div>
