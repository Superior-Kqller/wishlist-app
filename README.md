...
Последний релиз и история изменений: [CHANGELOG.md](CHANGELOG.md).

## Содержание

- [Что умеет](#-что-умеет)
- [Скриншоты](#-скриншоты)
- [Запуск за пару минут](#-запуск-за-пару-минут)
- [Переменные окружения](#-переменные-окружения)
- [Первый вход](#-что-делать-после-первого-входа)
- [Telegram-бот](#-telegram-бот)
- [Установка на телефон](#-установка-на-телефон)
- [Как это устроено](#-как-это-устроено)
- [FAQ](#-faq)
- [Self-host](#-коротко-для-self-host)
...
Основной шаблон лежит в [.env.example](.env.example). Для Docker Compose достаточно скопировать его в `.env` и заменить значения под свой домен, БД и стартовых пользователей.

| Переменная | Обязательна | Описание |
| --- | --- | --- |
| `DB_PASSWORD` | Да | Пароль пользователя PostgreSQL `wishlist` внутри Docker Compose. |
| `DATABASE_URL` | Да | Строка подключения Prisma к PostgreSQL. В production compose переопределяется на внутренний адрес `wishlist-db`. |
| `NEXTAUTH_SECRET` | Да | Секрет NextAuth для подписи сессий. Используйте длинную случайную строку. |
| `NEXTAUTH_URL` | Да | Публичный адрес приложения, например `https://wishlist.example.com`. Для локальной проверки можно указать `http://localhost:4030`. |
| `AUTH_TRUST_HOST` | Нет | Включайте при работе за reverse proxy, если хост приходит через proxy-заголовки. |
| `TELEGRAM_BOT_TOKEN` | Нет | Токен Telegram-бота от BotFather. Нужен только для Telegram-интеграции. |
| `TELEGRAM_WEBHOOK_SECRET` | Нет | Секрет, который Telegram передаёт в `x-telegram-bot-api-secret-token`. |
| `REDIS_URL` | Нет | Подключение к Valkey/Redis для rate limit. В compose используется unix-сокет `/run/valkey/valkey.sock`; без Valkey есть in-memory fallback. |
| `APP_PORT` | Нет | Локальный порт публикации контейнера. По умолчанию `4030`. |
| `SEED_USER1_*`, `SEED_USER2_*` | Да для первого запуска | Стартовые пользователи, которые создаются seed-скриптом при первичной настройке. |
...
Production compose публикует приложение только на loopback-интерфейсе:

```text
127.0.0.1:${APP_PORT:-4030}:4030
```

Это значит, что наружу его обычно отдаёт reverse proxy на том же сервере. Proxy должен принимать HTTPS-трафик на вашем домене и проксировать его на `http://127.0.0.1:4030`.
...
Для Caddy тот же сценарий короче:
...
После настройки proxy укажите внешний адрес в `NEXTAUTH_URL`. Если приложение работает за proxy и авторизация ведёт себя нестабильно, проверьте proxy-заголовки и при необходимости включите `AUTH_TRUST_HOST=true`.
...
Приложение собрано на Next.js App Router. Страницы и API-маршруты находятся в `src/app`, переиспользуемые UI-компоненты — в `src/components`, общая бизнес-логика — в `src/lib`.

Основные части системы:

- **Next.js + React** — интерфейс, серверные API-маршруты, PWA manifest и production build.
- **NextAuth** — авторизация пользователей, роли `USER` и `ADMIN`, сессии приложения.
- **Prisma + PostgreSQL** — модели пользователей, подборок, желаний, тегов, комментариев и Telegram-привязок.
- **Valkey/Redis** — rate limit и кэш с fallback на память процесса.
- **Telegram webhook** — обработка `/start`, inline-действий, бронирования и уведомлений.
- **Uploads** — загруженные изображения лежат в `public/uploads`, в Docker Compose каталог вынесен в volume `uploads-data`.

Короткий путь запроса:

```text
Браузер или Telegram → Next.js route/API → auth/access policy → Prisma → PostgreSQL
```

Для пользовательских страниц данные загружаются через API и SWR, а состояние фильтров синхронизируется с URL.
