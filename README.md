...
<p align="center">
  <img src="assets/app-demo.png" alt="Вишлист — главный экран со списком желаний и карточками" width="920">
</p>

<p align="center"><em>Главный экран: карточки, фильтры, поиск</em></p>

<p align="center">
  <img src="assets/add-smth.png" alt="Вишлист — форма добавления товара" width="920">
</p>

<p align="center"><em>Добавление товара (в т.ч. заполнение по ссылке)</em></p>
...
- 🤖 Telegram-бот: привязка аккаунта, быстрый просмотр и действия со статусами подарков
...
- Привязка Telegram к аккаунту через поле `Telegram ID` на странице `/settings`.
- Подтверждение привязки через команду `/start` в боте.
- Быстрые команды в боте:
  - `/myitems` — мои подарки;
  - `/available` — доступные подарки для брони.
- Действия из Telegram: бронь, снятие брони, пометка куплено.
- Уведомления о брони/покупке (если включены в настройках профиля).
...
При установке webhook в Telegram передавайте тот же `TELEGRAM_WEBHOOK_SECRET` как `secret_token`, чтобы запросы валидировались по заголовку `x-telegram-bot-api-secret-token`.
