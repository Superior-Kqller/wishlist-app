...
- Тёмная палитра стала спокойнее: фон и карточки теперь графитовые, а фиолетовый используется как акцент для действий.
- Приоритеты получили отдельную цветовую шкалу, поэтому важность желания легче считывается в карточках и формах.
- Статус на карточке перенесён вниз изображения и лучше читается поверх фотографий.
