...
function assertSafeSeedUsernames(env = process.env) {
  ...
  try {
    ...
    if (userCount === 0) {
      ...
      await prisma.user.create({
        data: { username: user1Username, password: hash1, name: user1Name, role: "ADMIN" },
      });

      await prisma.user.create({
        data: { username: user2Username, password: hash2, name: user2Name, role: "USER" },
      });
